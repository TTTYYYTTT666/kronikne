...
export default function EkspertizPage() {
    ...
    const [checklist, setChecklist] = useState<ChecklistItem[]>(
        baseChecklist.map(item => ({ ...item, checked: false }))
    );
    ...
    const handleSelectCar = (car: Car) => {
        ...
        setChecklist([...baseChecklist.map(item => ({ ...item, checked: false })), ...carSpecificChecks]);
    };
            ...
            <div className="max-w-4xl mx-auto px-4 py-12">
                ...
                <div className="text-center mb-12">
                    <h1 className="text-4xl font-bold mb-4">
                        ✅ Ekspertiz <span className="text-emerald-400">Checklist</span>
                    </h1>
                ...
                <div className="bg-zinc-900 rounded-2xl p-6 mb-8 border border-zinc-800">
                    ...
                    <div className="relative">
                        <input
                            ...
                            className="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-xl text-white placeholder-gray-500 focus:border-emerald-500 focus:outline-none"
                        />
                        {showDropdown && (
                            <div className="absolute z-10 w-full mt-2 bg-zinc-800 border border-zinc-700 rounded-xl max-h-60 overflow-y-auto">
                    ...
                    {selectedCar && (
                        <div className="mt-4 p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl flex items-center gap-4">
                            <div className="w-12 h-12">
                                <BrandLogo brand={selectedCar.brand} />
                            </div>
                            <div>
                                <div className="font-bold">{selectedCar.brand} {selectedCar.model} {selectedCar.variant}</div>
                                <div className="text-sm text-emerald-400">Araç özel kontrolleri eklendi!</div>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-zinc-900 rounded-2xl p-6 mb-8 border border-zinc-800">
                    <div className="flex justify-between items-center mb-3">
                        ...
                        <span className="text-emerald-400 font-bold">{checkedCount}/{checklist.length}</span>
                    </div>
                ...
                <div className="space-y-6">
                    {categories.map(category => (
                        <div key={category} className="bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800">
                            <div className="p-4 bg-zinc-800 font-bold">{category}</div>
                            <div className="divide-y divide-zinc-800">
                                {checklist.filter(item => item.category === category).map(item => (
                                    <button
                                        key={item.id}
                                        onClick={() => toggleCheck(item.id)}
                                        className={`w-full p-4 flex items-start gap-4 text-left transition-colors ${item.checked ? 'bg-emerald-500/10' : 'hover:bg-zinc-800/50'
                                            }`}
                                    >
                ...
                <div className="mt-8 text-center">
                    <button
                        ...
                        className="px-8 py-4 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl transition-colors"
                    >
                        🖨️ Listeyi Yazdır
                    </button>
                </div>
