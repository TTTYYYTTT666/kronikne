...
export async function getExperienceStats() {
    try {
        ...
        return {
            ...
            brands: brands.map(b => b.carBrand),
        };
    } catch (error) {
...
export async function addExperience(formData: FormData) {
    ...
    const pros = data.pros ? data.pros.split(',').map(p => DOMPurify.sanitize(p.trim(), { ALLOWED_TAGS: [], ALLOWED_ATTR: [] })).filter(Boolean).slice(0, 10) : [];
    const cons = data.cons ? data.cons.split(',').map(c => DOMPurify.sanitize(c.trim(), { ALLOWED_TAGS: [], ALLOWED_ATTR: [] })).filter(Boolean).slice(0, 10) : [];
